...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total bitcore balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CBitCoreAddress(addr).ToString());
...
#ifdef ENABLE_WALLET
        // Case 1: BitCore address and we have full public key:
        CBitCoreAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are bitcore addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) bitcore address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CBitCoreAddress address(innerID);
