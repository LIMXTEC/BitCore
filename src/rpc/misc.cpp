...
UniValue spork(const JSONRPCRequest& request)
{
    if(request.params.size() == 1 && request.params[0].get_str() == "show"){
        ...
        // FXTC BEGIN
        for(int nSporkID = SPORK_BTX_START; nSporkID <= SPORK_BTX_END; nSporkID++){
            if(sporkManager.GetSporkNameByID(nSporkID) != "Unknown")
                ret.push_back(Pair(sporkManager.GetSporkNameByID(nSporkID), sporkManager.GetSporkValue(nSporkID)));
        }
        // FXTC END
...
static UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given bitcore address.\n"
            "DEPRECATION WARNING: Parts of this command have been deprecated and moved to getaddressinfo. Clients must\n"
            "transition to using getaddressinfo to access this information before upgrading to v0.18. The following deprecated\n"
            "fields have moved to getaddressinfo and will only be shown here with -deprecatedrpc=validateaddress: ismine, iswatchonly,\n"
            "script, hex, pubkeys, sigsrequired, pubkey, addresses, embedded, iscompressed, account, timestamp, hdkeypath, kdmasterkeyid.\n"
            "\nArguments:\n"
            "1. \"address\"                    (string, required) The bitcore address to validate\n"
            "\nResult:\n"
            "{\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("validateaddress", "\"fExDspm4Jxk6NcLmwm2gDBREYngUn4QhbA\"")
            + HelpExampleRpc("validateaddress", "\"fExDspm4Jxk6NcLmwm2gDBREYngUn4QhbA\"")
        );
...
static UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitcore address to use for the signature.\n"
            ...
            "\nAs json rpc\n"
            + HelpExampleRpc("verifymessage", "\"fExDspm4Jxk6NcLmwm2gDBREYngUn4QhbA\", \"signature\", \"my message\"")
        );
...
static UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "bitcore-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\nbitcore-cli has the option -getinfo to collect and format these in the old format."
    );
}
