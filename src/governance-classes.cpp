...
bool CSuperblockManager::IsSuperblockTriggered(int nBlockHeight)
{
    ...
    for (auto pSuperblock : vecTriggers)
    {
        if(!pSuperblock) {
...
bool CSuperblockManager::IsValid(const CTransactionRef txNew, int nBlockHeight, CAmount blockReward, CBlockHeader pblock)
{
    ...
    CSuperblock_sptr pSuperblock;
    if(CSuperblockManager::GetBestSuperblock(pSuperblock, nBlockHeight)) {
        return pSuperblock->IsValid(txNew, nBlockHeight, blockReward, pblock);
    }
...
CAmount CSuperblock::GetPaymentsLimit(int nBlockHeight, CBlockHeader pblock)
{
    ...
    CAmount nSuperblockPartOfSubsidy = GetBlockSubsidy(nBlockHeight, pblock, consensusParams, true);
...
void CSuperblock::ParsePaymentSchedule(std::string& strPaymentAddresses, std::string& strPaymentAmounts)
{
    ...
    */
    ...
    for (int i = 0; i < (int)vecParsed1.size(); i++) {
        ...
        CTxDestination destination = DecodeDestination(vecParsed1[i]);
        ...
        if(payment.IsValid()) {
            ...
            vecPayments.clear();
            std::ostringstream ostr;
            ostr << "CSuperblock::ParsePaymentSchedule -- Invalid payment found: address = " << EncodeDestination(destination)
                 << ", amount = " << nAmount;
            LogPrintf("%s\n", ostr.str());
            throw std::runtime_error(ostr.str());
        }
...
bool CSuperblock::IsValid(const CTransactionRef txNew, int nBlockHeight, CAmount blockReward, CBlockHeader pblock)
{
    // TODO : LOCK(cs);
    ...
    CAmount nPaymentsLimit = GetPaymentsLimit(nBlockHeight, pblock);
