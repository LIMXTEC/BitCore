...
###   bitcorecore.org and bitcore.org and compares them.
...
if [ $? -ne 0 ]; then
   echo "bitcore.org failed to provide signature file, but bitcorecore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcore.org and bitcorecore.org signature files were not equal?"
