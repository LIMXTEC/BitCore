...
The RPM spec file provided here is for BitCore-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcore.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitCore git repository](https://github.com/bitcore-project/bitcore-core) but are not part of
...
Currently only `Source10` is used, for the example `bitcore.conf` file.
...
in the bitcore git repository. At present only an SVG version of the BitCore
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcore
...
needed when 0.12.1 is released, a proper fix is already in the BitCore git
master branch.
...
idea to build BitCore Core against a static version of that library compiled
...
The `bitcore.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcore-server` package. At this point, most RPM based
...
The `bitcore-server` package should have SELinux support. How to properly do
...
BitCore-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcore/). He has been
packaging BitCore for Fedora at least since 2012.
...
4. I renamed the `bitcore` package that contains the Qt GUI to `bitcore-core` as
that appears to be how the general population refers to it, in contrast to
`bitcore-xt` or `bitcore-classic`. I wanted to make sure the general population
