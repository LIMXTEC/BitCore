...
with the bitcored(s) being tested (using python's asyncore package); the other
implements the test logic.

- `NodeConn` is the class used to connect to a bitcored.  If you implement
...
behavior of a bitcored against 1 or more other bitcored instances. It should not be used
...
option to change the bitcored binary used by the test.  If using 2 or more nodes,
then `--refbinary` can be optionally used to change the bitcored that will be used
on nodes 2 and up.
...
Each `TestInstance` consists of:
  - A list of `[object, outcome, hash]` entries
    * `object` is a `CBlock`, `CTransaction`, or
    ...
    complete headers-chains when requested from the bitcored, to allow writing
    tests where blocks can be delivered out of order but still processed by
    headers-first bitcored's.
    * `outcome` is `True`, `False`, or `None`.  If `True`
    or `False`, the tip is compared with the expected tip -- either the
    block passed in, or the hash specified as the optional 3rd entry.  If
    `None` is specified, then the test will compare all the bitcored's
  ...
  - `sync_every_transaction`: `True/False`.  Analogous to
    ...
    then the contents of the entire mempool are compared across all bitcored
...
Taken from the [python-bitcorerpc repository](https://github.com/jgarzik/python-bitcorerpc).
