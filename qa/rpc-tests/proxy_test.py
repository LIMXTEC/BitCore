...
'''
Test plan:
- Start bitcored's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on bitcored side:
...
- Create bitcoreds that connect to them
- Manipulate the bitcoreds using addnode (onetry) an observe effects
...
class ProxyTest(BitCoreTestFramework):
    def __init__(self):
        super().__init__()
        self.num_nodes = 4
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: bitcored's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
